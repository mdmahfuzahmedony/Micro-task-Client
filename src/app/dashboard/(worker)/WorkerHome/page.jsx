      ...
      const fetchData = async () => {
        try {
          ...
          const [statsRes, tasksRes] = await Promise.all([
            axios.get(
              `https://micro-task-server-nine.vercel.app/worker-stats/${session.user.email}`
            ),
            axios.get(
              `https://micro-task-server-nine.vercel.app/worker-approved-tasks/${session.user.email}`
            ),
          ]);
    ...
    <div className="p-6 space-y-8 bg-slate-50 dark:bg-slate-900 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border dark:border-slate-700">
          <p className="text-slate-500 dark:text-slate-400 font-bold uppercase text-xs tracking-wider">
            Total Submission
          </p>
          <h2 className="text-4xl font-black mt-2 dark:text-white">
