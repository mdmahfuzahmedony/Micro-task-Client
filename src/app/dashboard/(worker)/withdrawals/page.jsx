...
const WithdrawalForm = () => {
  ...
  useEffect(() => {
    if (session?.user?.email) {
      axios
        .get(
          `https://micro-task-server-nine.vercel.appuser-stats/${session.user.email}`
        )
        .then((res) => {
  ...
  const handleWithdraw = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://micro-task-server-nine.vercel.app/withdrawals",
        withdrawData
      );
        ...
        <form onSubmit={handleWithdraw} className="space-y-6">
          ...
          {userCoins >= MIN_WITHDRAW_COIN ? (
            <button
              type="submit"
              disabled={
                isSubmitting || withdrawCoin > userCoins || withdrawCoin <= 0
              }
