...
const MySubmissions = () => {
  ...
  useEffect(() => {
    ...
    if (status === "authenticated" && user?.email) {
      axios
        .get(
          `https://micro-task-server-nine.vercel.app/my-submissions/${user.email}`
        )
        .then((res) => {
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium">
            Monitoring your work progress and earnings.
          </p>
        </div>
        ...
        {loading ? (
              ...
              <table className="w-full text-left border-separate border-spacing-0">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-900/50">
                    <th className="p-6 text-xs font-black text-slate-400 uppercase tracking-widest border-b dark:border-slate-800">
                      Task Title
                    </th>
                    <th className="p-6 text-xs font-black text-slate-400 uppercase tracking-widest border-b dark:border-slate-800">
                      Reward
                    </th>
                    <th className="p-6 text-xs font-black text-slate-400 uppercase tracking-widest border-b dark:border-slate-800 text-center">
                      Status
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {currentItems.map((sub) => {
                    ...
                    return (
                      <tr
                        key={sub._id}
                        className="hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-all"
                      >
                        <td className="p-6">
                        ...
                        <td className="p-6">
                          <div className="flex justify-center">
                            <span
                              className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-wider border ${style.bg} ${style.text} ${style.border}`}
                            >
          ...
          <div className="flex-grow flex flex-col items-center justify-center py-20 px-4 text-center">
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-2 max-w-xs mx-auto text-sm">
              You havent submitted any tasks yet. Browse the task list and start
              earning coins today.
            </p>
          </div>
        )}
