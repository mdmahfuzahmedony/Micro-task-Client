...
const ReviewTask = () => {
  ...
  const handleApprove = async (id, worker_email, amount) => {
    const res = await axios.patch(
      `https://micro-task-server-nine.vercel.appapprove-submission/${id}`,
      {
        worker_email,
        amount,
      }
  ...
  const handleReject = async (id, task_id) => {
    const res = await axios.patch(
      `https://micro-task-server-nine.vercel.appreject-submission/${id}`,
      { task_id }
    );
    if (res.data.success) {
      Swal.fire(
  ...
  return (
    <div className="p-6 bg-slate-50 dark:bg-slate-900 min-h-screen">
      <h2 className="text-2xl font-bold mb-6 text-slate-800 dark:text-white uppercase">
        Task To Review
      </h2>
