...
const PaymentHistory = () => {
    ...
    useEffect(() => {
        if (session?.user?.email) {
            axios.get(`http://localhost:5000/payment-history/${session?.user?.email}`)
                .then(res => {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center min-h-[60vh]">
            ...
            <p className="mt-4 font-medium text-slate-500 animate-pulse">Loading transaction records...</p>
        </div>
    );
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
                    <div>
                        <h2 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white flex items-center gap-3">
                            ...
                            Payment <span className="text-indigo-600 dark:text-indigo-400">History</span>
                        </h2>
                    ...
                    <div className="bg-white dark:bg-slate-900 px-6 py-3 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xl font-black text-slate-800 dark:text-white">{payments.length}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-200 dark:border-slate-800 overflow-hidden">
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50 dark:bg-slate-800/50 border-b dark:border-slate-800">
                                    ...
                                    <th className="px-6 py-5 text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Transaction ID</th>
                                    <th className="px-6 py-5 text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Coins</th>
                                    <th className="px-6 py-5 text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Amount</th>
                                    <th className="px-6 py-5 text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Date</th>
                                    <th className="px-6 py-5 text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-center">Status</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {payments.map((payment, index) => (
                                    <tr key={payment._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
                                        <td className="px-6 py-5 font-medium text-slate-400">{index + 1}</td>
                                        ...
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="font-black text-slate-700 dark:text-slate-200">{payment.coins}</span>
                                            </div>
                                        </td>
                    ...
                    {payments.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-20 text-center">
                            ...
                            <h3 className="text-xl font-bold text-slate-800 dark:text-white">No Transactions Yet</h3>
                            <p className="text-slate-500 dark:text-slate-400 mt-2">When you purchase coins, they will appear here.</p>
                        </div>
                    )}
                </div>
            </div>
