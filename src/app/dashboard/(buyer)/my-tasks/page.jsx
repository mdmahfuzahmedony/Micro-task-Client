...
const Mytask = () => {
  ...
  useEffect(() => {
    if (session?.user?.email) {
      fetch(
        `https://micro-task-server-nine.vercel.app/my-tasks/${session?.user?.email}`
      )
        .then((res) => res.json())
        .then((data) => {
          setTasks(data);
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch(
            `https://micro-task-server-nine.vercel.app/task/${id}`,
            {
              method: "DELETE",
            }
          );
          if (res.ok) {
            ...
            Swal.fire("Error", "Could not delete the task from server.", "error");
          }
        } catch (error) {
      ...
      {tasks.length === 0 ? (
        <div className="bg-white p-20 text-center rounded-3xl shadow-sm border border-dashed border-slate-300">
          <p className="text-slate-400 font-bold">
            No tasks found. Create one to see it here!
          </p>
        </div>
      ) : (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 uppercase tracking-widest">
                          Total Cost: ${task.total_payable_amount}
                        </p>
                      </div>
                    </div>
                  </td>
