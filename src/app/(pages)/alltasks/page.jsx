...
const AllTasks = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get("https://micro-task-server-nine.vercel.appall-tasks")
      .then((res) => {
        setTasks(res.data);
        setLoading(false);
      })
  ...
  const filteredTasks = tasks
    .filter((task) =>
      filterCategory
        ? task.task_title.toLowerCase().includes(filterCategory.toLowerCase())
        : true
    )
    .sort((a, b) => {
          ...
          <div className="flex-1">
            {loading ? (
              ...
              <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-[2rem] border border-dashed border-slate-200 dark:border-slate-800">
                <p className="text-slate-400 italic text-lg font-medium">
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm transition-all">
                ...
                <div className="space-y-2">
                  {["", "YouTube", "Facebook", "Survey", "App Download"].map(
                    (cat) => (
                      <button
                        key={cat}
