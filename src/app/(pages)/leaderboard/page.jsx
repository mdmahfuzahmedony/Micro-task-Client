...
const Leaderboard = () => {
  ...
  const filteredUsers = users.filter((user) => {
    const matchesCountry =
      filterCountry === "Global" || user.country === filterCountry;
      ...
      <div className="max-w-[1600px] mx-auto px-6 lg:px-20">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-lg text-slate-600 dark:text-slate-400">
            Celebrate the success of our most hardworking community members from
            around the world.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16 items-end">
          ...
          {topThree[1] && (
            <div className="order-2 md:order-1 bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 text-center relative pt-16 h-[350px] flex flex-col justify-center shadow-lg">
              ...
              <p className="text-3xl font-black text-slate-500">
                {topThree[1].balance} <span className="text-sm">Coins</span>
              </p>
              <div className="mt-6 inline-block bg-slate-100 dark:bg-slate-800 px-4 py-2 rounded-full text-sm font-bold text-slate-500">
                2nd Place
              </div>
            </div>
          )}
          ...
          {topThree[0] && (
            <div className="order-1 md:order-2 bg-gradient-to-b from-blue-600 to-indigo-700 p-10 rounded-[3rem] text-center relative pt-20 h-[420px] flex flex-col justify-center shadow-2xl shadow-blue-500/20 scale-105 z-10">
              ...
              <Trophy
                className="absolute top-12 right-12 text-yellow-400 animate-bounce"
                size={48}
              />
              <h3 className="text-3xl font-bold text-white mb-1">
                {topThree[0].name}
              </h3>
              <p className="text-blue-100 font-medium mb-6 opacity-80">
                {topThree[0].country}
              </p>
              <p className="text-5xl font-black text-white">
                {topThree[0].balance} <span className="text-lg">Coins</span>
              </p>
              <div className="mt-8 inline-block bg-yellow-400 text-blue-900 px-6 py-2 rounded-full text-md font-black uppercase tracking-widest">
                Champion
              </div>
            </div>
          )}
          ...
          {topThree[2] && (
            <div className="order-3 bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 text-center relative pt-16 h-[320px] flex flex-col justify-center shadow-lg">
              ...
              <Award
                className="absolute top-10 right-10 text-amber-600"
                size={32}
              />
              <h3 className="text-2xl font-bold text-slate-900 dark:text-white">
                {topThree[2].name}
              ...
              <p className="text-3xl font-black text-amber-700">
                {topThree[2].balance} <span className="text-sm">Coins</span>
              </p>
              <div className="mt-6 inline-block bg-amber-50 dark:bg-amber-900/20 px-4 py-2 rounded-full text-sm font-bold text-amber-600">
                3rd Place
              </div>
            </div>
          )}
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 overflow-hidden shadow-sm">
          <table className="w-full text-left">
            <thead className="bg-slate-50 dark:bg-slate-800/50">
              <tr>
                <th className="p-6 text-sm font-bold text-slate-500 uppercase tracking-widest">
                  Rank
                </th>
                <th className="p-6 text-sm font-bold text-slate-500 uppercase tracking-widest">
                  User
                ...
                <th className="p-6 text-sm font-bold text-slate-500 uppercase tracking-widest text-right">
                  Total Coins
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
              {restOfUsers.map((user, index) => (
                ...
                >
                  <td className="p-6 font-black text-slate-400">
                    #{index + 4}
                  </td>
                  ...
                  <td className="p-6 text-right font-black text-emerald-500 text-xl">
